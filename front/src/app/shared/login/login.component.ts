    ...
    this.userService.login(this.form.get('username').value, this.form.get('password').value)
      .subscribe(res => {
          if (res) {
            ...
            this.snackBar.open('Logged in. You will be redirected in a moment', '', {duration: 6000, panelClass: 'green-snackbar'});
          }
        }, err => {
          this.isRequestSend = false;
          this.snackBar.open('Incorrect ID or password', '', {duration: 3000, panelClass: 'red-snackbar'});
        }
      );
  }
